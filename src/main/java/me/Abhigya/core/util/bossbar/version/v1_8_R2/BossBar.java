package me.Abhigya.core.util.bossbar.version.v1_8_R2;
...
/** A {@code BossBar} intended for the server version {@link Version#v1_8_R2}. */
public class BossBar extends BossBarOldest {
    ...
    protected synchronized void create() {
        ...
        data.watch(
                6,
                Math.min(Math.max((float) getProgress(), MINIMUM_PROGRESS), MAXIMUM_PROGRESS)
                        * handle.getMaxHealth());
        data.watch(8, (byte) 0);
        data.watch(17, 0);
        data.watch(18, 0);
    ...
    protected synchronized void update() {
        if (this.handle == null
                || (handle != null
                        && !Objects.equals(
                                getPlayer().getWorld().getUID(),
                                handle.world.getWorld().getUID()))) {
            this.create();
            return;
        }
        ...
        BukkitReflection.sendPacket(
                getPlayer(), new PacketPlayOutEntityMetadata(this.handle.getId(), data, true));
    ...
    protected synchronized void destroy() {
        ...
        if (player != null && player.isOnline()) {
            BukkitReflection.sendPacket(
                    player, new PacketPlayOutEntityDestroy(this.handle.getId()));
        }
