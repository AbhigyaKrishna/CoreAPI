...
public abstract class BossBar {
    ...
    public static BossBar createBossBar(
        ...
        if (Version.getServerVersion().isNewerEquals(Version.v1_9_R1)) {
            final BossBar bossbar =
                    new me.Abhigya.core.util.bossbar.version.latest.BossBar(
                            title, progress, player);
        ...
        try {
            final Class<?> c0 =
                    Class.forName(
                            VERSIONS_PACKAGE + Version.getServerVersion().name() + ".BossBar");
    ...
    /**
     * Creates a boss bar instance to display to players. The progress defaults to {@code 1.0}.
     *
     * <p>
     *
     * @param player Player viewer
     ...
     */
    public static BossBar createBossBar(
            Player player, String title, BarColor color, BarStyle style, BarFlag... flags) {
        return createBossBar(player, title, 1F, color, style, flags);
    }

    /**
     * Creates a boss bar instance to display to players. The color defaults to {@link
     * BarColor#PINK}. The style defaults to {@link BarStyle#SOLID}.
     *
     * <p>
     *
     * @param player Player viewer
    ...
    /**
     * Creates a boss bar instance to display to players. The color defaults to {@link
     * BarColor#PINK}. The style defaults to {@link BarStyle#SOLID}. The progress defaults to {@code
     * 1.0}.
     *
     * <p>
     *
     ...
     *
     * <p>This is the equivalent of calling {@link #setVisible(boolean)} providing <strong>{@code
     * false}</strong>.
     */
    public void show() {
        setVisible(true);
