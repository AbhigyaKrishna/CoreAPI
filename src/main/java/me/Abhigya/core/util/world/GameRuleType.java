...
/** Enumeration helper class for all {@link org.bukkit.GameRule} */
public enum GameRuleType {
    ...
    public GameRuleMetadata getGameRuleMetadata(World world) {
        Validate.isTrue(
                getPresentationMode() == GameRulePresentationMode.METADATA,
                "wrong presentation mode!");

        for (MetadataValue metadata :
                world.getMetadata(getName()).stream()
    ...
    public boolean isPresent(World world) {
        return getPresentationMode() != GameRulePresentationMode.METADATA
                || getGameRuleMetadata(world) != null;
    }
        ...
        if (getPresentationMode() == GameRulePresentationMode.ORDINARY) {
            try {
                ...
                switch (getDataType()) {
                    ...
                    default:
                        world.setGameRule(
                                (org.bukkit.GameRule<Boolean>)
                                        org.bukkit.GameRule.getByName(getName()),
                                (Boolean) value);
                        break;

                    case INTEGER:
                        world.setGameRule(
                                (org.bukkit.GameRule<Integer>)
                                        org.bukkit.GameRule.getByName(getName()),
                                (Integer) value);
                        break;
                }
            ...
            world.setMetadata(
                    getName(),
                    new GameRuleMetadata(CoreAPI.getInstance().getHandlingPlugin(), this, value));
        }
    }
}
