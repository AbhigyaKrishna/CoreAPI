...
public class StereoMode extends ChannelMode {
    ...
    public void play(
        ...
        if (!song.isStereo() && fallbackChannelMode != null) {
            fallbackChannelMode.play(
                    player, location, song, layer, note, soundCategory, volume, pitch);
            return;
        }
    ...
    public void play(
        ...
        if (!song.isStereo() && fallbackChannelMode != null) {
            fallbackChannelMode.play(
                    player, location, song, layer, note, soundCategory, volume, doTranspose);
            return;
        }
    ...
    /**
     * Returns scale of panning in blocks. {@link Note} with maximum left panning will be played
     * this distance from {@link Player}'s head on left side.
     *
     * @return
     */
    ...
    /**
     * Sets scale of panning in blocks. {@link Note} with maximum left panning will be played this
     * distance from {@link Player}'s head on left side.
     *
     * @param maxDistance
     */
    ...
    /**
     * Sets fallback {@link ChannelMode} which is used when song is not stereo. Set to null to
     * disable.
     *
    ...
    public void setFallbackChannelMode(ChannelMode fallbackChannelMode) {
        if (fallbackChannelMode instanceof StereoMode)
            throw new IllegalArgumentException(
                    "Fallback ChannelMode can't be instance of StereoMode!");
