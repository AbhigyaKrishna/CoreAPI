...
public class HologramFactory extends PluginHandler {
    ...
    public Hologram createHologram(String id, Location location, String... lines) {
        if (this.hasHologram(id)) return null;
    ...
    public Hologram getHologram(String id) {
        if (this.holograms.containsKey(id)) return this.holograms.get(id);
    ...
    //                if
    // (hologram.getLocation().getWorld().getName().equals(event.getTo().getWorld().getName())) {
        ...
        for (Hologram hologram : this.holograms.values()) {
            if (hologram.isSpawned()) {
                if (hologram.getLocation()
                    ...
                    if (hologram.canSee(event.getPlayer()))
                        hologram.spawn(Collections.singleton(event.getPlayer()));
                } else if (event.getFrom()
                        .getName()
                        .equals(hologram.getLocation().getWorld().getName())) {
                    if (hologram.canSee(event.getPlayer())) hologram.safeDestroy(event.getPlayer());
                }
            }
        }
    }
    ...
    private void handleChunkUnload(ChunkUnloadEvent event) {
        for (Hologram hologram : this.holograms.values()) {
            if (hologram.isSpawned()) {
                if (hologram.getLocation().getBlockX() >> 4 == event.getChunk().getX()
                        && hologram.getLocation().getBlockZ() >> 4 == event.getChunk().getZ()) {
                    for (Player player : hologram.getViewers()) hologram.safeDestroy(player);
                }
            }
        }
    }
            ...
            if (hologram.isSpawned()) {
                if (hologram.getLocation().getBlockX() >> 4 == event.getChunk().getX()
                    ...
                    for (Player player : event.getWorld().getPlayers()) {
                        if (hologram.canSee(player)) hologram.spawn(Collections.singleton(player));
                    }
                }
            }
        }
