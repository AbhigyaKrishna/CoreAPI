package me.Abhigya.core.util.yaml;
...
/**
 * An implementation of {@link Configuration} which saves all files in Yaml, and
 * allows to comments the keys by using the method {@link #options()}.comment().
 * <p>
...
//			System.out.println ( ">>>> node parent section = " + ( section != null ? section.getCurrentPath ( ) : "null" ) );
...
//				System.out.println ( ">>>> 2: section path = " + section.getCurrentPath ( ) );
                ...
                Map<String, String> comments = options().getComments().get(section.getCurrentPath());

                if (comments != null) {
                    for (String key : comments.keySet()) {
                        if (line_path.equalsIgnoreCase(key)) {
//							System.out.println ( ">>>> - (FOUND) " + key + ": " + comments.get ( key ) );
                        } else {
//							System.out.println ( ">>>> - " + key + ": " + comments.get ( key ) );
                ...
                if (comment != null) {
                    builder.append(getIndent(indent) + YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX + comment);
                    builder.append(LINE_SEPARATOR);
                }

//				if ( options ( ).isCommented ( section , line_path ) ) {
////					System.out.println ( ">>>> 3" );
//					builder.append ( getIndent ( indent ) + YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX
...
//	}
    ...
    protected void loadComments(String input) {
        ...
        for (int i = 0; i < lines.length; i++) {
                    ...
                    for (int j = i - 1; j >= 0; j--) {
                        ...
                        if (line_back_indent == (indent - options().indent())) {
                            ...
                            if (last_node.contains(String.valueOf(options().pathSeparator()))) {
                                ...
                                if (last_node.substring(0, last_node.lastIndexOf(options().pathSeparator()))
                                        .endsWith(options().pathSeparator() + line_back_path)) {
                                    node = last_node.substring(0,
                                            last_node.lastIndexOf(options().pathSeparator()))
                                            + options().pathSeparator() + line_path;
                                }
                            }
            ...
            if (!comment_line.trim().startsWith(YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX)
                    || comment_line.trim().length() == YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX
                    .length()) {
                continue;
            }

            String comment = comment_line.trim()
                    .substring(YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX.length()).trim();
    ...
    public YamlConfigurationCommentsOptions options() {
        return (YamlConfigurationCommentsOptions) (options == null
                ? (options = new YamlConfigurationCommentsOptions(this)) : options);
    }
}
