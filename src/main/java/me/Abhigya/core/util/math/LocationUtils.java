...
public class LocationUtils {
    ...
    public static String format(
            Location location, boolean append_yaw, boolean append_pitch, boolean append_world) {
    ...
    public static Set<Block> getBlocksBetween(Location corner_a, Location corner_b) {
        if (corner_a.getWorld() != corner_b.getWorld()) {
            throw new IllegalArgumentException(
                    "cannot get blocks between "
                            + corner_a.getWorld().getName()
                            + " and "
    ...
    public static boolean isInsideWorldBorder(final Location location, final WorldBorder border) {
        try {
            ...
            final Class<?> block_position_class =
                    ClassReflection.getNmsClass("BlockPosition", "core");
            final Constructor<?> block_position_constructor =
                    block_position_class.getConstructor(double.class, double.class, double.class);
            ...
            final Object block_position =
                    block_position_constructor.newInstance(
            ...
            return (location.getWorld().equals(world))
                    && (boolean) (a.invoke(handle, block_position));
        } catch (Throwable t) {
            t.printStackTrace();
            return false;
        }
