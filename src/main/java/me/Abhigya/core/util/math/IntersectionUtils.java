...
public class IntersectionUtils {
    ...
    private static final Vector v2 = new Vector();

    /**
     * Intersects a {@link Ray} and a {@link BoundingBox}, returning the intersection point in
     * intersection. This intersection is defined as the point on the ray closest to the origin
     * which is within the specified bounds.
     *
     * <p>The returned intersection (if any) is guaranteed to be within the bounds of the bounding
     * box, but it can occasionally diverge slightly from ray, due to small floating-point errors.
     *
     * <p>If the origin of the ray is inside the box, this method returns true and the intersection
     * point is set to the origin of the ray, accordingly to the definition above.
     *
     * @param ray The ray
     * @param box The box
    ...
    public static boolean intersectRayBounds(Ray ray, BoundingBox box, Vector intersection) {
        ...
        if (ray.getOrigin().getX() <= box.getMinimum().getX() && ray.getDirection().getX() > 0) {
            ...
            if (t >= 0) {
                ...
                if (v2.getY() >= box.getMinimum().getY()
                        && v2.getY() <= box.getMaximum().getY()
                        && v2.getZ() >= box.getMinimum().getZ()
                        && v2.getZ() <= box.getMaximum().getZ()
                        && (!hit || t < lowest)) {
                    hit = true;
                    lowest = t;
                }
        ...
        if (ray.getOrigin().getY() <= box.getMinimum().getY() && ray.getDirection().getY() > 0) {
            ...
            if (t >= 0) {
                set(v2, ray.getDirection()).multiply(t).add(ray.getOrigin());
                if (v2.getX() >= box.getMinimum().getX()
                        && v2.getX() <= box.getMaximum().getX()
                        && v2.getZ() >= box.getMinimum().getZ()
                        && v2.getZ() <= box.getMaximum().getZ()
                        && (!hit || t < lowest)) {
                    hit = true;
     ...
     * @return Whether the ray and the bounding box intersect
     */
    public static boolean intersectRayBoundsFast(Ray ray, Vector center, Vector dimensions) {
    ...
    /**
     * Quick check whether the provided {@link Ray} and {@link BoundingBox} intersects.
     *
     * <p>
     *
     * @param ray Ray
    ...
    private static Vector set(Vector v0, Vector v1) {
        v0.setX(v1.getX()).setY(v1.getY()).setZ(v1.getZ());
        return v0;
    }
}
