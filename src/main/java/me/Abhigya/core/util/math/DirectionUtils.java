...
public class DirectionUtils {
     ...
     *
     * <p>Also <strong>{@code NaN}</strong> will be returned if the provided block face is not a 90°
     * face. It means that only the following block faces are supported:
     *
     * <ul>
    ...
    /**
     * Gets the euler angles from the direction vector result of the subtraction of {@code to} and
     * {@code from}.
     *
     * <p>
     *
    ...
    public static float[] lookAt(Vector from, Vector to) {
        ...
        final double dst_xz =
                Math.sqrt(NumberConversions.square(dx) + NumberConversions.square(dz));
        final double dst_y =
                Math.sqrt(NumberConversions.square(dst_xz) + NumberConversions.square(dy));
    ...
    /**
     * Gets a unit-vector pointing in the direction represented by the specified euler angles
     ...
     * @return Vector pointing the direction represented by the specified euler angles.
     */
    ...
    public static Block getLeft(Block block, BlockFace direction, int numBlocks) {
        ...
        return block.getRelative(
                bf.getModX() * numBlocks, bf.getModY() * numBlocks, bf.getModZ() * numBlocks);
    }
