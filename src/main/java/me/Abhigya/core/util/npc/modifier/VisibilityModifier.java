...
public class VisibilityModifier extends NPCModifier {
     ...
     */
    @NotNull
    public VisibilityModifier queuePlayerListChange(
            @NotNull WrappedPacketOutPlayerInfo.PlayerInfoAction action) {
        WrappedPacketOutPlayerInfo.PlayerInfo playerInfoData =
                new WrappedPacketOutPlayerInfo.PlayerInfo(
                        "", super.npc.getGameProfile(), GameMode.CREATIVE, 20);
        ...
        //        packetContainer.getPlayerInfoDataLists().write(0, new
        // ArrayList<>(Arrays.asList(playerInfoData)));
    ...
    public VisibilityModifier queueSpawn() {
        ...
        //            vector = new Vector3d(Math.floor(x * 32.0D), Math.floor(y * 32.0D),
        // Math.floor(z * 32.0D));
        //        } else {
        ...
        WrappedPacketOutNamedEntitySpawn packetContainer =
                new WrappedPacketOutNamedEntitySpawn(
                        this.npc.getEntityId(),
                        super.npc.getProfile().getUniqueId(),
                        vector,
                        super.npc.getLocation().getYaw(),
                        super.npc.getLocation().getPitch());
        this.packetContainers.add(packetContainer);
    ...
    public VisibilityModifier queueDestroy() {
        ...
        //            packetContainer.getIntLists().write(0, new
        // ArrayList<>(Arrays.asList(super.npc.getEntityId())));
        //        } else {
        //            packetContainer.getIntegerArrays().write(0, new
        // int[]{super.npc.getEntityId()});
        //        }
        return this;
    }
