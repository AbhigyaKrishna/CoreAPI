package me.Abhigya.core.util.saveable;
            ...
            if (entry.isAnnotationPresent(SaveableEntry.class)) {
                ...
                if (destiny_section != null) {
                    if (blank_key) {
                        ...
                        switch (options.action()) {
                            case NOT_EQUAL:
                                count += YamlUtils.setNotEqual(destiny_section, options.key(), value) ? 1 : 0;
                                break;
                            case NOT_SET:
                                count += YamlUtils.setNotSet(destiny_section, options.key(), value) ? 1 : 0;
                                break;
                            case NORMAL:
                            default:
                ...
                for (Saveable item : (Collection<Saveable>) value) {
                    item.save(sub_section.createSection(
                            StringUtils.defaultIfBlank(options.subsectionprefix(), "") + String.valueOf(item_count++)));
                }
            }
        }
        return count;
