...
public class MaterialUtils {
    ...
    static {
        ...
        try {
            FROM_LEGACY_DATA_PRIORITY =
                    CRAFT_MAGIC_NUMBERS_CLASS.getMethod(
                            "fromLegacy", MaterialData.class, boolean.class);
    ...
    public static Material getRightMaterial(ItemStack stack) {
        try {
            if (isLegacy(stack.getType())) {
                return (Material)
                        FROM_LEGACY_DATA_PRIORITY.invoke(
                                CRAFT_MAGIC_NUMBERS_CLASS, stack.getData(), true);
            }
        } catch (Throwable t) {
            /* ignore */
        }
    ...
    /**
     * Creates a new BlockData instance for the specified Material, with all properties initialized
     * to unspecified defaults.
     *
     * <p>
     *
