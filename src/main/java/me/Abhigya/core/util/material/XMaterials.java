...
public enum XMaterials {
    ...
    */
    ...
    END_PORTAL("ENDER_PORTAL", null, null, null, null, "END_PORTAL"),
    ...
    private static final HashMap<String, XMaterials> inUMemory = new HashMap<>();
    ...
    XMaterials(String... names) {
        this(0, names);
    }

    XMaterials(String name, int data) {
        names[0] = name;
        versionName = name;
        this.data = (byte) data;
    }

    XMaterials(String name, int spoofedData, Color color) {
        names[0] = name;
        versionName = name;
    ...
    XMaterials(PotionBase base, String type, boolean extended, boolean upgraded, String... names) {
        this.names = names;
        attributes = "upotion=" + base.name() + ":" + type + ":" + extended + ":" + upgraded;
    }

    XMaterials(String name, Enchantment enchant, int level) {
        names[0] = name;
        versionName = name;
        data = 0;
    ...
    XMaterials(int data, String... names) {
        this.names = names;
        this.data = (byte) data;
    }
    ...
    @Deprecated
    public static XMaterials match(String name, byte data) {
        name = name.toUpperCase();
        if (inUMemory.containsKey(name + data)) return inUMemory.get(name + data);
        for (XMaterials u : XMaterials.values()) {
            if (u.getData() == data) {
    ...
    public static XMaterials matchSpawnEgg(ItemStack egg) {
        if (egg != null) {
    ...
    public static XMaterials matchEnchantedBook(ItemStack book) {
    ...
    public static XMaterials matchPotion(ItemStack potion) {
    ...
    public static XMaterials match(ItemStack item) {
        if (item != null && !item.getType().equals(Material.AIR)) {
            ...
            if (version.contains("1.8") || version.contains("1.9") || version.contains("1.10")
                ...
                final XMaterials u = match(un, d), potion = u == null ? matchPotion(item) : null;
                un = u != null ? u.name() : potion != null ? potion.name() : null;
                if (un != null)
                    return valueOf(un);
            }
            return un != null ? un.contains("SPAWN_EGG") ? matchSpawnEgg(item)
                    : un.contains("ENCHANTED_BOOK") ? matchEnchantedBook(item)
                    : un.contains("POTION") ? matchPotion(item) : valueOf(un)
                    : null;
        }
        return null;
    }

    public static XMaterials getItem(Block block) {
        ...
        if (!v.contains("1.8") && !v.contains("1.9") && !v.contains("1.10") && !v.contains("1.11")
            ...
            if (d == 8) {
                t = XMaterials.ACACIA_WOOD;
            } else if (d == 12) {
                t = log2 ? XMaterials.ACACIA_WOOD : XMaterials.OAK_WOOD;
            } else if (d == 13) {
                t = log2 ? XMaterials.DARK_OAK_WOOD : XMaterials.SPRUCE_WOOD;
            } else if (d == 14) {
                t = XMaterials.BIRCH_WOOD;
            } else if (d == 15) {
                t = XMaterials.JUNGLE_WOOD;
            }
            ...
            if (v.contains("1.12")) {
                ...
                return XMaterials.WHITE_BED;
        } else if (m.contains("CROP")) {
            return XMaterials.AIR;
        } else if (m.contains("PISTON_")) {
            return m.contains("_STICKY_") ? XMaterials.STICKY_PISTON : XMaterials.PISTON;
...
class UPotion {
    ...
    private final XMaterials.PotionBase base;
    private final ItemStack potion;
    private final PotionType type;
    private final Object potiondata;

    public UPotion(XMaterials.PotionBase base, String type, boolean extended, boolean upgraded) {
        this.base = base;
    ...
    public XMaterials.PotionBase getBase() {
        return base;
    }
