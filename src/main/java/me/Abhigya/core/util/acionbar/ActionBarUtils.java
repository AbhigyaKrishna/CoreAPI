...
public class ActionBarUtils {
     ...
     *
     * <p>Note that the length {@code message} message must be <strong>{@code < 63}</strong>, if the
     * length of the provided message is higher than <strong>{@code 63}</strong>, that it will
     * limited.
     *
     * <p>
     *
    ...
    public static void send(Player player, String message) {
        try {
            ...
            Class<?> chat_serializer =
                    inner_classes.length > 0
                            ? component_class.getClasses()[0]
                            : ClassReflection.getNmsClass(
                                    "ChatSerializer", "network.chat.IChatBaseComponent");
            Class<?> packet_class =
                    ClassReflection.getNmsClass("PacketPlayOutChat", "network.protocol.game");

            Object component =
                    MethodReflection.get(chat_serializer, "a", String.class)
                            .invoke(
                                    chat_serializer,
            ...
            if (CoreAPI.getInstance().getServerVersion().isOlder(Version.v1_12_R1)) {
                ...
                Class<?> chat_type_class =
                        ClassReflection.getNmsClass("ChatMessageType", "network.chat");
                ...
                packet =
                        ConstructorReflection.newInstance(
                                ...
                                component,
                                MethodReflection.get(chat_type_class, "valueOf", String.class)
                                        .invoke(chat_type_class, "GAME_INFO"));
            }
    ...
    /**
     * Sends an action bar with the provide {@code message} to all the players online.
     *
     * <p>
     *
     * @param message Message to send.
