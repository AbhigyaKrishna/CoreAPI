...
public class UUIDEntity<T extends Entity> {
    ...
    /**
     * Construct the {@code UUIDEntity} from its {@link UUID unique id} and its type class.
     *
     * <p>
     *
    ...
    /**
     * Gets the respective entity associated with the entity's {@link #uuid}. This will look in all
     * the worlds for an entity whose its {@link UUID unique id} is the same as this.
     *
     * <p>
     *
    ...
    public T get() {
        ...
        for (World world : Bukkit.getWorlds()) {
            Optional<T> filter =
                    world.getEntitiesByClass(getTypeClass()).stream()
                            .filter(entity -> entity.getUniqueId().equals(getUniqueId()))
                            .findAny();
            if (filter.isPresent()) {
                return filter.get();
            }
