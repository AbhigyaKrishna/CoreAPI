...
public class TitleUtils {
    ...
    /**
     * Sends a title and a subtitle message to the player. If either of these values are null, they
     * will not be sent and the display will remain unchanged. If they are empty strings, the
     * display will be updated as such. If the strings contain a new line, only the first line will
     * be sent. All timings values may take a value of -1 to indicate that they will use the last
     * value sent (or the defaults if no title has been displayed).
     *
     * <p>
     *
    ...
    public static void send(
            Player player, String title, String subtitle, int fade_in, int stay, int fade_out) {
        try {
            ...
            Object packet0 =
                    ConstructorReflection.get(packet_class, int.class, int.class, int.class)
                            .newInstance(fade_in, stay, fade_out); // times packet construction
            Object packet1 =
                    ConstructorReflection.get(packet_class, action_enum, component_class)
    ...
    /**
     * Resets the title displayed to the player. This will clear the displayed title/subtitle and
     * reset timings to their default values.
     *
     * <p>
     *
     * @param player Target player.
    ...
    public static void reset(Player player) {
        try {
            ...
            Object action =
                    MethodReflection.get(action_enum, "valueOf", String.class)
                            .invoke(action_enum, "RESET");
            Object packet =
                    ConstructorReflection.get(packet_class, action_enum, component_class)
    ...
    public static void broadcast(
            String title, String subtitle, int fade_in, int stay, int fade_out) {
        Bukkit.getOnlinePlayers()
                .forEach(
                        player ->
                                TitleUtils.send(player, title, subtitle, fade_in, stay, fade_out));
    }
