    ...
    public static void tryHook(Plugin plugin) {
        ...
        if (event.getPlaceholderManager() != null) {
            ...
            ConsoleUtils.sendPluginMessage(
                    ChatColor.GREEN, "Successfully hooked with custom PlaceholderManager", plugin);
        } else if (Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
            manager = new PAPIPlaceholderManager();
            ConsoleUtils.sendPluginMessage(
                    ChatColor.GREEN, "Successfully hooked into PlaceholderAPI", plugin);
        } else {
            manager = new CustomPlaceholderManager();
            ConsoleUtils.sendPluginMessage(
                    ChatColor.YELLOW, "Cannot find PlaceholderAPI, using internal system.", plugin);
            fallback = true;
