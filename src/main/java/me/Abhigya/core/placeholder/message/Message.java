...
public class Message {
                            ...
                            .getValue().stream()
                                    .map(
                                            lineNode -> {
                                                ...
                                                return PlaceholderValue.stringValue(
                                                        ((ScalarNode) lineNode).getValue());
                                            })
    ...
    public static Message fromConfig(Object obj) {
        ...
        if (obj instanceof Collection) {
            ...
            return new Message(
                    Collections.singletonList(PlaceholderValue.stringValue(obj.toString())));
        }
    }
        ...
        if (node.getTag() != expected) {
            throw new IllegalStateException(
                    "Wrong type: found "
                            + node.getTag()
                            + ", expected: "
                            + Arrays.stream(new Tag[] {expected})
                                    .map(Tag::getValue)
        ...
        if (!expected.contains(node.getTag())) {
            throw new IllegalStateException(
                    "Wrong type: found "
                            + node.getTag()
                            + ", expected: "
                            + Arrays.stream(expected.toArray(new Tag[0]))
                                    .map(Tag::getValue)
                                    .collect(Collectors.joining(", ")));
        }
    }
    ...
    public List<String> get(Player player, PlaceholderRegistry<?> placeholders) {
        return lines.stream()
                .map(p -> p.resolve(player, placeholders))
                .collect(Collectors.toList());
    }

    public List<String> get(Player player, String k1, String v2) {
        return get(player, PlaceholderRegistry.wrap(k1, v2));
    }
    ...
    public List<String> get(
            Player player, String k1, String v1, String k2, String v2, String k3, String v3) {
        return get(
                player,
                PlaceholderRegistry.wrap(
                        k1, v1,
    ...
    public Message filter(String k1, String v1) {
        return filter(PlaceholderRegistry.wrap(k1, v1));
    }
    ...
    public void send(Player player, String k1, String v1) {
        send(player, PlaceholderRegistry.wrap(k1, v1));
    }
    ...
    public void send(CommandSender sender, PlaceholderRegistry<?> placeholders) {
        for (PlaceholderValue<String> message : lines)
            sender.sendMessage(
                    StringUtils.translateAlternateColorCodes(message.resolve(null, placeholders)));
    }

    public void send(CommandSender sender, String k1, String v1) {
        send(sender, PlaceholderRegistry.wrap(k1, v1));
    }
    ...
    public void broadcast(Iterable<? extends Player> players, String k1, String v1) {
        broadcast(players, PlaceholderRegistry.wrap(k1, v1));
    }

    public void broadcast(
            Iterable<? extends Player> players, String k1, String v1, String k2, String v2) {
        broadcast(
                players,
                PlaceholderRegistry.wrap(
                        k1, v1,
    ...
    public void broadcast(String k1, String v1) {
        broadcast(Bukkit.getOnlinePlayers(), PlaceholderRegistry.wrap(k1, v1));
    }
